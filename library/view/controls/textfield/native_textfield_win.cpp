...
namespace view
{
    ...
    NativeTextfieldWin::ScopedFreeze::ScopedFreeze(NativeTextfieldWin* edit,
        ITextDocument* text_object_model) : edit_(edit),
        ...
        bg_color_(0)
    {
        ...
        DWORD ex_style = ui::GetExtendedStyles();
    ...
    bool NativeTextfieldWin::IsNumPadDigit(int key_code, bool extended_key)
    {
        ...
        return !extended_key &&
            ((key_code>=VK_PRIOR && key_code<=VK_DOWN) || // All keys but 5 and 0.
    ...
    HWND NativeTextfieldWin::GetTestingHandle() const
    {
        return m_hWnd;
    }
    ...
    bool NativeTextfieldWin::IsIMEComposing() const
    {
        ...
        const int composition_size = ImmGetCompositionString(imm_context,
            GCS_COMPSTR, NULL, 0);
    ...
    bool NativeTextfieldWin::HandleKeyPressed(const KeyEvent& event)
    {
        return false;
    }

    bool NativeTextfieldWin::HandleKeyReleased(const KeyEvent& event)
    {
        return false;
    }
    ...
    void NativeTextfieldWin::OnCopy()
    {
        ...
        if(!text.empty() && ViewDelegate::view_delegate)
        {
            ui::ScopedClipboardWriter scw(
                ViewDelegate::view_delegate->GetClipboard());
            scw.WriteText(text);
        }
    }
        ...
        WPARAM wparam, LPARAM lparam)
    {
        ...
        ime_discard_composition_ = language_id==LANG_JAPANESE ||
            language_id==LANG_CHINESE;
        ime_composition_start_ = 0;
        ime_composition_length_ = 0;
    ...
    void NativeTextfieldWin::OnNonLButtonDown(UINT keys, const gfx::Point& point)
    {
