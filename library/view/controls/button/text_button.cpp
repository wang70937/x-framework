...
namespace view
{
    ...
    // static
    const char TextButtonBase::kViewClassName[] = "view/TextButtonBase";
    // static
    const char TextButton::kViewClassName[] = "view/TextButton";
    // static
    const char NativeTextButton::kViewClassName[] = "view/NativeTextButton";
    ...
    {
        insets->Set(kPreferredPaddingVertical,
            kPreferredPaddingHorizontal,
            kPreferredPaddingVertical,
            kPreferredPaddingHorizontal);
    }
    ...
    void TextButtonBase::UpdateTextSize()
    {
        ...
        max_text_size_.SetSize(std::max(max_text_size_.width(), text_size_.width()),
            std::max(max_text_size_.height(), text_size_.height()));
        PreferredSizeChanged();
    }
    ...
    gfx::Rect TextButtonBase::GetContentBounds(int extra_width) const
    {
        ...
        case ALIGN_RIGHT:
            ...
            break;
        case ALIGN_CENTER:
            content_x = insets.left() + std::max(0, (available_width-content_width)/2);
            break;
        }
        content_width = std::min(content_width, width()-insets.right()-content_x);
    ...
    void TextButtonBase::PaintButton(gfx::Canvas* canvas, PaintButtonMode mode)
    {
        ...
        if(text_bounds.width() > 0)
        {
            ...
            {
                SkColor shadow_color = GetWidget()->IsActive() ?
                    active_text_shadow_color_ :
    ...
    gfx::Size TextButton::GetPreferredSize()
    {
        ...
        if(!ignore_minimum_size_)
        {
            ...
            prefsize.set_height(std::max(prefsize.height(),
    ...
    NativeTextButton::NativeTextButton(ButtonListener* listener,
        const std::wstring& text) : TextButton(listener, text)
    {
        Init();
    }
