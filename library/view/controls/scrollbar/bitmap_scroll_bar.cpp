...
namespace view
{
    ...
    namespace
    {
        ...
        private:
            void NotifyClick()
            {
                ...
                MouseEvent event(ui::ET_MOUSE_RELEASED, cursor_point.x(),
                    cursor_point.y(), ui::EF_LEFT_BUTTON_DOWN);
                Button::NotifyClick(event);
            }
        ...
        public:
            ...
            virtual gfx::Size GetPreferredSize()
            {
                return gfx::Size(background_bitmap()->width(),
                    start_cap_bitmap()->height()+end_cap_bitmap()->height()+
    ...
    gfx::Rect BitmapScrollBar::GetTrackBounds() const
    {
        ...
        if(IsHorizontal())
        {
            ...
            int new_width = std::max(0, width()-(prefsize.width()*2));
    ...
    {
        contents_scroll_offset_ = CalculateContentsOffset(thumb_position,
            scroll_to_middle);
        ...
        const gfx::Point& p, bool is_mouse_gesture)
    {
        ...
        scoped_ptr<Menu> menu(Menu::Create(this, Menu::TOPLEFT,
    ...
    std::wstring BitmapScrollBar::GetLabel(int id) const
    {
        ...
        return ids_value ? UTF16ToWide(ui::GetStringUTF16(ids_value)) : L"";
    }
    ...
    void BitmapScrollBar::ButtonPressed(Button* sender, const Event& event)
    {
