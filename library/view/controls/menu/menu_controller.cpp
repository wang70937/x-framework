...
namespace view
{
        ...
        void Update(const MenuController::MenuPart& part)
        {
            ...
            if(!scrolling_timer_.IsRunning())
            {
                scrolling_timer_.Start(
                    base::TimeDelta::FromMilliseconds(kScrollTimerMS),
    ...
    private:
        void Run()
        {
            ...
            vis_rect.set_y(is_scrolling_up_ ? std::max(0, start_y_-delta_y) :
        ...
        int* result_mouse_event_flags)
    {
        ...
        {
            ViewDelegate::view_delegate->AddRef();
        }
        ...
        if(ViewDelegate::view_delegate)
        {
            ViewDelegate::view_delegate->ReleaseRef();
        }
        ...
        const MouseEvent& event)
    {
        ...
        if(part.type==MenuPart::NONE || (part.type==MenuPart::MENU_ITEM &&
            part.menu &&
    ...
    bool MenuController::GetDropFormats(SubmenuView* source,
        int* formats,
    ...
    bool MenuController::CanDrop(SubmenuView* source, const ui::OSExchangeData& data)
    {
        ...
        int selection_types)
    {
        ...
        MenuDelegate* current_delegate = current_path.empty() ? NULL :
            current_path.front()->GetDelegate();
    ...
    bool MenuController::OnKeyDown(int key_code, const MSG& msg)
    {
        DCHECK(blocking_run_);
        ...
        MenuItemView::AnchorPosition position)
    {
        ...
        pending_state_.monitor_bounds = ui::Screen::GetMonitorWorkAreaNearestPoint(
            bounds.origin());
    }
        ...
        const MouseEvent& event)
    {
        ...
        HWND window_under_mouse = ui::Screen::GetWindowAtCursorScreenPoint();
        ...
        UpdateInitialLocation(gfx::Rect(screen_menu_loc.x(), screen_menu_loc.y(),
            button->width(), button->height()-1), anchor);
    ...
    MenuController::MenuPart MenuController::GetMenuPartByScreenCoordinateUsingMenu(
        MenuItemView* item, const gfx::Point& screen_loc)
    {
        MenuPart part;
        while(item)
        {
    ...
    void MenuController::OpenMenuImpl(MenuItemView* item, bool show)
    {
        ...
        bool prefer_leading = state_.open_leading.empty() ? true :
            state_.open_leading.back();
        bool resulting_direction;
        gfx::Rect bounds = CalculateMenuBounds(item, prefer_leading,
            &resulting_direction);
    ...
    {
        show_timer_.Start(base::TimeDelta::FromMilliseconds(kShowDelay),
    ...
    {
        cancel_all_timer_.Start(base::TimeDelta::FromMilliseconds(kCloseOnExitTime),
        ...
        if(!item->GetParentMenuItem())
        {
                ...
                y+pref.height()>state_.monitor_bounds.bottom())
            {
                ...
                if(item->actual_menu_position()==MenuItemView::POSITION_BELOW_BOUNDS ||
                    ...
                    y<(state_.monitor_bounds.y()+state_.monitor_bounds.height()/2)))
                {
    ...
    void MenuController::UpdateActiveMouseView(SubmenuView* event_source,
        const MouseEvent& event, View* target_menu)
    {
        View* target = NULL;
        ...
        if(target != active_mouse_view_)
        {
            ...
            if(active_mouse_view_)
            {
                ...
                MouseEvent mouse_pressed_event(ui::ET_MOUSE_PRESSED,
                    target_point.x(), target_point.y(), event.flags());
