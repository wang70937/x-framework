...
namespace view
{
    ...
    namespace
    {
        ...
        static int WanGetMenuItemID(HMENU hMenu, int pos)
        {
        ...
        public:
            MenuHostWindow(MenuWin* menu, HWND parent_window) : menu_(menu)
            {
                ...
                {
                    extended_style |= ui::GetExtendedStyles();
                }
            ...
            VIEW_BEGIN_MSG_MAP_EX(MenuHostWindow);
                VIEW_MSG_WM_RBUTTONUP(OnRButtonUp)
                VIEW_MSG_WM_MEASUREITEM(OnMeasureItem)
                VIEW_MSG_WM_DRAWITEM(OnDrawItem)
            VIEW_END_MSG_MAP();
        ...
        private:
            ...
            void OnRButtonUp(UINT w_param, const gfx::Point& loc)
            {
                int id;
                if(menu_->delegate() && FindMenuIDByLocation(menu_, loc, &id))
                {
                    menu_->delegate()->ShowContextMenu(menu_, id, loc, true);
                }
            }
            ...
            void OnMeasureItem(WPARAM w_param, MEASUREITEMSTRUCT* lpmis)
            {
                ...
                {
                    gfx::Font font;
                    lpmis->itemWidth = font.GetStringWidth(data->label) +
            ...
            void OnDrawItem(UINT wParam, DRAWITEMSTRUCT* lpdis)
            {
                ...
                if(lpdis->itemData)
                {
                    ...
                    gfx::Font font;
                    HGDIOBJ old_font = static_cast<HFONT>(SelectObject(hDC,
                        font.GetNativeFont()));
            ...
            bool FindMenuIDByLocation(MenuWin* menu, const gfx::Point& loc, int* id)
            {
                int index = MenuItemFromPoint(NULL, menu->menu_, loc.ToPOINT());
                if(index != -1)
                {
                    *id = WanGetMenuItemID(menu->menu_, index);
                    return true;
                }
                else
                {
    ...
    // static
    Menu* Menu::Create(Delegate* delegate, AnchorPoint anchor, HWND parent)
    {
        return new MenuWin(delegate, anchor, parent);
    }

    // static
    Menu* Menu::GetSystemMenu(HWND parent)
    {
        return new MenuWin(::GetSystemMenu(parent, FALSE));
    }
    ...
    MenuWin::MenuWin(Delegate* d, AnchorPoint anchor, HWND owner)
        ...
        owner_draw_(ui::NeedOverrideDefaultUIFont(NULL, NULL))
    {
        DCHECK(delegate());
    }
    ...
    bool MenuWin::SetIcon(const SkBitmap& icon, int item_id)
    {
        ...
        for(int i=0; i<num_items; ++i)
        {
            ...
            {
                if(WanGetMenuItemID(menu_, i) == item_id)
                {
    ...
    void MenuWin::RunMenuAt(int x, int y)
    {
        ...
        UINT flags = GetTPMAlignFlags() | TPM_LEFTBUTTON | TPM_RETURNCMD | TPM_RECURSE;
        ...
        UINT selected_id = TrackPopupMenuEx(menu_, flags, x, y,
            active_host_window->hwnd(), NULL);
        if(created_host)
        {
    ...
    void MenuWin::SetMenuInfo()
    {
        ...
        for(int i=0; i<num_items; ++i)
        {
            ...
            {
                const int id = WanGetMenuItemID(menu_, i);
    ...
    UINT MenuWin::GetStateFlagsForItemID(int item_id) const
    {
        ...
        UINT flags = delegate()->IsCommandEnabled(item_id) ? MFS_ENABLED
            : MFS_DISABLED;
