...
namespace view
{
    ...
    namespace internal
    {
        ...
        public:
            ...
            virtual void DropMenuClosed(NotifyType type, MenuItemView* menu);
            virtual void SiblingMenuCreated(MenuItemView* menu);
            ...
            int32 types)
        {
            ...
            // Run the loop.
            MenuItemView* result = controller->Run(parent, button, menu_,
                bounds, anchor, &mouse_event_flags);

            if(for_drop_)
            {
                // Drop menus return immediately.
                // We finish processing in DropMenuClosed.
                return MenuRunner::NORMAL_EXIT;
            }
        ...
        bool MenuRunnerImpl::ShouldShowMnemonics(MenuButton* button)
        {
            ...
            // We don't currently need this on gtk as showing the menu gives
