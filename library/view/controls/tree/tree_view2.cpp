...
namespace view
{
    ...
    void TreeView2::GetAccessibleState(ui::AccessibleViewState* state)
    {
        state->role = ui::AccessibilityTypes::ROLE_OUTLINE;
        state->state = ui::AccessibilityTypes::STATE_READONLY;
    }

    void TreeView2::SetModel(ui::TreeModel* model)
    {
        if(model == model_)
        {
            return;
    ...
    void TreeView2::SetEditable(bool editable)
    {
    ...
    void TreeView2::StartEditing(ui::TreeModelNode* node)
    {
    ...
    ui::TreeModelNode* TreeView2::GetSelectedNode()
    {
        if(!tree_view_)
        {
            return NULL;
    ...
    void TreeView2::Expand(ui::TreeModelNode* node)
    {
        DCHECK(model_ && node);
    ...
    void TreeView2::ExpandAll(ui::TreeModelNode* node)
    {
        DCHECK(node);
        // Expand the node.
    ...
    bool TreeView2::IsExpanded(ui::TreeModelNode* node)
    {
        if(!tree_view_)
        {
            return false;
    ...
    void TreeView2::SetRootShown(bool root_shown)
    {
    ...
    void TreeView2::TreeNodesAdded(ui::TreeModel* model,
    ...
    void TreeView2::TreeNodesRemoved(ui::TreeModel* model,
        ui::TreeModelNode* parent,
        int start,
        int count)
    {
    ...
    void TreeView2::TreeNodeChanged(ui::TreeModel* model, ui::TreeModelNode* node)
    {
...
/*
    gfx::Point TreeView2::GetKeyboardContextMenuLocation()
    {
        int y = height() / 2;
        if(GetSelectedNode())
        {
    ...
    HWND TreeView2::CreateNativeControl(HWND parent_container)
    {
    ...
    LRESULT TreeView2::OnNotify(int w_param, LPNMHDR l_param)
    {
        switch(l_param->code)
        {
    ...
    void TreeView2::OnDestroy()
    {
        Cleanup();
    }

    bool TreeView2::OnKeyDown(ui::KeyboardCode virtual_key_code)
    {
    ...
    void TreeView2::OnContextMenu(const POINT& location)
    {
...
*/
    ui::TreeModelNode* TreeView2::GetNodeForTreeItem(HTREEITEM tree_item)
    {
        NodeDetails* details = GetNodeDetailsByTreeItem(tree_item);
        return details ? details->node : NULL;
    }

    HTREEITEM TreeView2::GetTreeItemForNode(ui::TreeModelNode* node)
    {
        NodeDetails* details = GetNodeDetails(node);
        return details ? details->tree_item : NULL;
    }

    void TreeView2::Cleanup()
    {
        RemoveObserverFromModel();
    ...
    void TreeView2::AddObserverToModel()
    {
    ...
    void TreeView2::RemoveObserverFromModel()
    {
        if(model_ && observer_added_)
        {
    ...
    void TreeView2::DeleteRootItems()
    {
    ...
    void TreeView2::CreateRootItems()
    {
        DCHECK(model_);
        DCHECK(tree_view_);
    ...
    void TreeView2::CreateItem(HTREEITEM parent_item,
    ...
    void TreeView2::RecursivelyDelete(NodeDetails* node)
    {
        DCHECK(node);
    ...
    TreeView2::NodeDetails* TreeView2::GetNodeDetails(ui::TreeModelNode* node)
    {
        DCHECK(node &&
    ...
    TreeView2::NodeDetails* TreeView2::GetNodeDetailsByID(int id)
    {
        DCHECK(id_to_details_map_.find(id) != id_to_details_map_.end());
        return id_to_details_map_[id];
    }

    TreeView2::NodeDetails* TreeView2::GetNodeDetailsByTreeItem(HTREEITEM tree_item)
    {
    ...
    HIMAGELIST TreeView2::CreateImageList()
    {
    ...
    HTREEITEM TreeView2::GetTreeItemForNodeDuringMutation(ui::TreeModelNode* node)
    {
        if(node_to_details_map_.find(node) == node_to_details_map_.end())
        {
            // User hasn't navigated to this entry yet. Ignore the change.
    ...
    LRESULT CALLBACK TreeView2::TreeWndProc(HWND window,
        UINT message,
        WPARAM w_param,
        LPARAM l_param)
    {
        ...
        DCHECK(wrapper);
        TreeView2* tree = wrapper->tree_view;
