...
namespace view
{
    ...
    void TableView::SetSortDescriptors(const SortDescriptors& sort_descriptors)
    {
        ...
        {
            ResetColumnSortImage(sort_descriptors_[0].column_id, NO_SORT);
        }
    ...
    int TableView::FirstSelectedRow()
    {
        ...
        int view_row = ListView_GetNextItem(list_view_, -1,
        ...
        if(is_visible)
        {
            ...
            if(column.min_visible_width == 0)
            {
                ...
                column.min_visible_width = ListView_GetStringWidth(list_view_,
                    column.title.c_str())+kListViewTextPadding;
            }
            changed = true;
        }
        if(changed)
        ...
        LPARAM l_param)
    {
        ...
        case WM_CONTEXTMENU:
            {
                    ...
                    (GET_X_LPARAM(l_param)!=-1 || GET_Y_LPARAM(l_param)!=-1))
                {
                        ...
                        PtInRect(&client_rect, table_point))
                    {
                        ...
                        int view_index = GetViewIndexFromPoint(window,
                            gfx::Point(table_point));
                        if(view_index != -1)
                        {
        ...
        return CallWindowProc(table_view->original_handler_, window,
            message, w_param, l_param);
    }
    ...
    HWND TableView::CreateNativeControl(HWND parent_container)
    {
        ...
        ui::AdjustUIFontForWindow(list_view_);
    ...
    bool TableView::SelectMultiple(int view_index, int mark_view_index)
    {
        ...
        for(int i=std::min(view_index, mark_view_index),
            max_i=std::max(view_index, mark_view_index); i<=max_i; ++i)
        {
            ...
            // just clicked in.
            if(model_->HasGroups() && model_->GetGroupID(ViewToModel(i))!=group_id)
            {
                continue;
            }
    ...
    int CALLBACK TableView::SortFunc(LPARAM model_index_1_p,
        LPARAM model_index_2_p, LPARAM table_view_param)
    {
    ...
    void TableView::ResetColumnSortImage(int column_id, SortDirection direction)
    {
        ...
        std::vector<int>::const_iterator i = std::find(visible_columns_.begin(),
            visible_columns_.end(), column_id);
    ...
    int TableView::CompareRows(int model_row1, int model_row2)
    {
        ...
        int sort_result = model_->CompareValues(model_row1, model_row2,
    ...
    int TableView::GetColumnWidth(int column_id)
    {
        ...
        return ListView_GetColumnWidth(list_view_,
            static_cast<int>(i-visible_columns_.begin()));
    }
                ...
                if(table_type_ == ICON_AND_TEXT)
                {
                    ...
                    if(!image.isNull())
                    {
                        ...
                        if(ListView_GetItemRect(list_view_, view_index, &icon_rect,
                            LVIR_ICON) && GetClientRect(list_view_, &client_rect))
                        {
                            RECT intersection;
                            ...
                            if(IntersectRect(&intersection, &icon_rect, &client_rect))
                            {
                                ...
                                int selected = ListView_GetItemState(list_view_,
                                    view_index, LVIS_SELECTED|LVIS_DROPHILITED);
                                ...
                                RECT to_draw =
                                {
                                    intersection.left-icon_rect.left,
            ...
            i!=visible_columns_.end(); ++i)
        {
            ...
            if(col.width == -1)
            {
                ...
                else
                {
                    ...
                    if(percent==0.f && available_width>0 && col_index==column_count_-1)
                    {
    ...
    void TableView::UpdateListViewCache0(int start, int length, bool add)
    {
        ...
        memset(&item, 0, sizeof(LVITEM));
        item.mask = (table_type_ == ICON_AND_TEXT) ? (LVIF_IMAGE | LVIF_TEXT) :
            LVIF_TEXT;
        item.stateMask = 0;
    ...
    TableSelectionIterator::TableSelectionIterator(TableView* view,
        int view_index) : table_view_(view), view_index_(view_index)
    {
