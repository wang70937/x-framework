...
#include "widget/native_widget_private.h"
#include "widget/native_widget_views.h"
...
namespace view
{
    ...
    {
        return base::i18n::IsRTL() ? (width()-bounds.x()-bounds.width()) :
            bounds.x();
    }
    ...
    gfx::Rect View::ConvertRectFromWidget(const gfx::Rect& rect) const
    {
    ...
    IAccessible* View::GetNativeViewAccessible()
    {
        if(!native_view_accessibility_win_.get())
        {
    ...
    void View::WriteDragData(const gfx::Point& press_pt, ui::OSExchangeData* data)
    {
    ...
    void View::RemoveDescendantToNotify(View* view)
    {
        ...
        Views::iterator i(find(descendants_to_notify_->begin(),
    ...
    bool View::ProcessMouseDragged(const MouseEvent& event, DragInfo* drag_info)
    {
        ...
        {
            if(!drag_controller_ || drag_controller_->CanStartDragForView(
