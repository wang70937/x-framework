...
namespace view
{
    ...
    bool FocusManager::OnKeyEvent(const KeyEvent& event)
    {
            ...
            key_code==ui::VKEY_LEFT || key_code==ui::VKEY_RIGHT))
        {
            ...
            View::Views views;
            focused_view_->parent()->GetViewsInGroup(
                focused_view_->GetGroup(), &views);
            View::Views::const_iterator i(std::find(views.begin(),
        ...
        if(original_starting_view)
        {
            ...
            if(!focus_traversable)
            {
                if(!reverse)
                {
                    ...
                    if(!focus_traversable)
                    {
                        focus_traversable = original_starting_view->GetWidget()->
                            GetFocusTraversable();
                        starting_view = original_starting_view;
                    }
                }
    ...
    void FocusManager::FocusNativeView(HWND native_view)
    {
